...
| 配置 | crop | distortion | 弯曲p95 | rough(残余抖动) |
|---|---|---|---|---|
| 经典基线（无权重） | 0.878 | 0.929 | 0.15px | 0.7148 |
| **全网络（出厂）** | 0.881 | 0.930 | **0.13px** | **0.6072** |

输入 rough 1.776 → 0.607（**−66%**）；输出恒为全局单应（角点空间），
**直线弯曲/果冻结构性为零**。
...
三档是同一权衡——**裁掉多少画幅 ↔ 换来多稳**——上的三个工作点,全部运行在
**角点空间**(输出恒为全局单应,直线弯曲结构性为零,果冻免疫)。参数取自
NUS 144 段全量实测(`--preset` 默认 `recommended`,日常无需指定):

| 档位 | crop | 保留画幅 | rough | 弯曲 p95 | distortion |
|---|---|---|---|---|---|
| `minimal_crop` | 0.08 | 0.922 | 0.687 | 0.102px | 0.937 |
| **`recommended`**(默认) | 0.12 | 0.881 | **0.607** | 0.125px | 0.930 |
| `most_stable` | 0.16 | 0.841 | 0.594 | 0.133px | 0.929 |

**拐点在 0.12**:0.08→0.12 rough 113/144 段改善(z=+6.83),越过后收益
掉进噪声(0.12→0.16 z=−0.17)。`most_stable` 的价值集中在 Running
(−10%)与 Parallax/Zooming 小幅,不是"更好的推荐档"。

> **相对旧顶点空间架构的取舍**(git 历史 `d15c40c` 为旧标定):全局
> rough +5.7%(0.574→0.607),视差类 +20~25%;换来弯曲 p95 从 5.6px
> 降到 0.13px——果冻从根上消失,且 distortion 显著更好(z=+7.00)。
> 在"果冻厌恶"的产品要求下这是正确工作点。真实高抖 4K 素材上 rough
> 代价更明显,详见评测文档。旧路径可用 `corner_space=False` 复现。
...
```bash
python -m pytest tests/ -q     # 82 个测试, 全部离线合成数据, CPU 可跑
```
...
- 果冻类**网格形变已结构性根除**（角点空间, 弯曲数学上为零）；素材自身
  的卷帘快门伪影不在校正范围内；
