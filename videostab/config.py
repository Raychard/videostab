...
class SmoothingConfig:
    ...
    # 出厂默认开启("极度厌恶果冻"的产品要求); False = 顶点空间旧路径,
    # NUS 全局 rough 略优(0.574 vs 0.607)但弯曲 5.6px, 仅供对照实验.
    corner_space: bool = True
...
#: 三档工作点(裁掉多少画幅 <-> 有多稳), 主轴 crop_ratio. 全部运行在
#: 角点空间(果冻免疫, 弯曲结构性为零), NUS 144 段全量标定(角点+双网络):
#:
#:   crop   cropping   rough   弯曲p95  distortion
#:   0.08    0.9215   0.6873    0.102     0.9369
#:   0.12    0.8806   0.6072    0.125     0.9303
#:   0.16    0.8408   0.5938    0.133     0.9293
#:
#: 拐点与顶点空间时代一致, 仍在 0.12: 0.08->0.12 rough 113/144 段改善
#: (z=+6.83), 0.12->0.16 掉进噪声 (z=-0.17). 顶点空间旧标定(τ 副轴等)
#: 见 git 历史 d15c40c; 角点模式没有各向异性可封, τ 不再是档位参数.
PRESETS = {
    # 保画幅优先: cropping 0.92, rough 代价 +13% (0.687 vs 0.607).
    "minimal_crop": dict(crop_ratio=0.08, corner_space=True),
    # 日常默认: 拐点值.
    "recommended": dict(crop_ratio=0.12, corner_space=True),
    # 高动态素材专用. 全局 rough 无显著差别 (z=-0.17), 收益集中在
    # Running -10% (0.835->0.753) 与 Parallax/Zooming 小幅; 多付 4 个
    # 百分点画幅. 不是"更好的推荐档".
    "most_stable": dict(crop_ratio=0.16, corner_space=True),
}
