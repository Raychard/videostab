...
def limit_anisotropy(B: np.ndarray, shape_hw: tuple, cap_ratio: float,
    ...
    这是预算硬约束与相似性保持之间的固有张力, 补救式迭代解决不了.

    "让预算投影本身保持相似结构"的方案已实测并否决: 逐帧整体等比缩放能
    精确保持相似结构(残差 ~1e-6), 但它衰减**所有**顶点的校正幅度, 而钳位
    只削超标顶点. 36 段 A/B: rough 0.4809->0.5263 显著变差(z=-2.33),
    distortion 无改善(z=-1.33). 失败是结构性的 —— 预算顶得越死缩放系数
    越小, 恰好在最需要校正处削得最多(Running 饱和率 100%: distortion
    确实 0.8896->0.9186, 但 rough 0.9158->1.1146). 结论: 逐顶点钳位造成
    的剪切, 代价小于均匀衰减损失的校正幅度. 该方向已关闭.
    """
